...
/**
 * <h1>Utils</h1>
 * <h3>Utility class for misc functions and operations needed to operate the
 * library code.</h3>
 *
 * @author Maxie D. Schmidt
 * @email maxieds@gmail.com
 * @since 8/8/2018 (partially modified from the Chameleon Mini Live Debugger
 *        source since 12/31/17)
 * @ref
 * @ref
 */
public class Utils {
    ...
    public static boolean FLAG_IS_OTHER(int flagParam) {
        return flagParam != FLAG_FALSE && flagParam != FLAG_TRUE;
    }
    ...
    public static byte MSB(byte[] bytesArray) {
        return bytesArray[0];
    }
    ...
    public static byte LSB(byte[] bytesArray) {
    ...
    /**
     * Generates a semi-random array of UID bytes with a pre-spefecified prefix set
     * of bytes.
     * 
     * @param prefixBytes
     * @param desiredArrayLength
     * @return null (if parameters are non-sensical) or a prefixed byte array with
     *         randomly generated suffix bits
     */
    public static byte[] generateRandomBytes(byte[] prefixBytes, int desiredArrayLength) {
        ...
        return ArrayUtils
                .toPrimitive(arrayJoin(ArrayUtils.toObject(prefixBytes), ArrayUtils.toObject(randomSuffixBytes)));
    }
    ...
    /**
     * Converts a string representation of a two-digit byte into a corresponding
     * byte type.
     * 
     * @param byteStr
    ...
    /**
     * Returns an ascii print character (or '.' representation for non-print
     * characters) of the input byte.
     * 
     * @param b
    ...
    /**
     * Truncates a long (hex or ascii string) so that it is more human readable
     * while still giving the gist of the output it represents.
     * 
     * @param str
     * @param maxNumChars
    ...
    /**
     * Joins two arrays of the same arbitrary type <AType>. The order of the joined
     * array is in the identical order of the passed parameters (i.e., full prefix
     * array first (MSB's) followed by the suffix array bits.
     * 
     * @param prefixArray
     * @param suffixArray
     * @param             <AType>
     ...
     * @ref Arrays.*
     * @return The joined array (or null if the expected parameters are not sanely
     *         defined)
     */
    ...
    /**
     * Determine whether an input string is in hex format and returned the clipped
     * hex-only part of the string if so (otherwise: returns null). For example,
    ...
    /****
     * Build Parameters: **** public static String getLibraryVersionCode() { return
     * BuildConfig.FULL_LIBRARY_NAME; }
     * 
     * public static String getLibraryBuildStamp() { return
