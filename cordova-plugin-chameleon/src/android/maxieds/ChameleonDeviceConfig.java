...
public class ChameleonDeviceConfig implements ChameleonUSBInterface {
    ...
    /****
     * This is the global Chameleon Board device configuration that should be
     * refered to from all instances in *both* this library code *AND* in the
     * customized day-to-day Android code being developed for the client bread
     * company! Create an array here if you end up needing more than one device
     * connected at a time, though I should mention that this in itself is a
     * complicated prospect since the RevE firmware does not really have any
     **** mechanism for uniquely identifying devices (i.e., only by vendor and product
     * IDs) on the fly. However, the RevG devices do have built-in unique device
     * identifiers, so this could be a possibility at some point if the specs grow
     * or change...
     ****/
     ...
     **** interacting with the Chameleon boards. They're aren't that many parameters to
     * configure here (except possibly the desired data transfer speed), however,
     * this is important in that it effectively controls setup/shutdown of the
     * serial communication. This should be used in conjunction with the running
     * ChameleonMiniUSBActivity to automatically configure this data --AND-- shut it
     * down correctly when the USB device is detected as detached by the running
     * Android OS. This combination is significantly more reliable for *long-term*
     * (re)use of a plugged in Chameleon device that will be used to ingest
     * *MULTIPLE* Mifare dumps over time rather than just a single plug-and-play use
     * followed by immediate disconnection:
     ****/

    /****
     * Constants and helper functions for determining the Chameleon Mini Revision
     * types:
     ****/
    ...
    public static enum ChameleonBoardType_t {
        REVE_REBOOTED, REVG, REVE_OTHER, REV_CUSTOM_FIRMWARE, REV_OLDER_FIRMWARE,
    };
    ...
    public static ChameleonBoardType_t getChameleonBoardType() {
        return localChameleonBoardRev;
    }

    public static String getChameleonBoardTypeByName() {
        return localChameleonBoardRev.name();
    }

    public static boolean isRevisionEDevice() {
        return localChameleonBoardRev == ChameleonBoardType_t.REVE_REBOOTED
                || localChameleonBoardRev == ChameleonBoardType_t.REVE_OTHER;
    }
    ...
    public static enum SerialUSBStates {
        IDLE, PAUSED, WAITING_FOR_RESPONSE, EXPECTING_BINARY_DATA, UPLOAD, WAITING_FOR_XMODEM_UPLOAD, DOWNLOAD,
        WAITING_FOR_XMODEM_DOWNLOAD, ERROR_OCCURED, UNEXPECTED_INCOMING_RXDATA,
    };
    ...
    public static UsbSerialDevice configureSerialPort(UsbSerialInterface.UsbReadCallback readerCallback) {
        ...
        if (device == null || connection == null) {
            ...
            PendingIntent permIntent = PendingIntent.getBroadcast((Context) mainApplicationActivity, 0,
        ...
        if (serialPort != null && serialPort.open()) {
            ...
            serialPort.setDataBits(USB_DATA_BITS); // slight optimization from UsbSerialInterface.DATA_BITS_8? ... yes,
                                                   // better
    ...
    public static UsbSerialInterface.UsbReadCallback usbReaderCallback = new UsbSerialInterface.UsbReadCallback() {
        ...
        public void onReceivedData(byte[] liveRxData) {
            ...
            // typically generate logs of the bytes in human-readable format for parsing
            // and/or verifying in realtime:
            ...
            if (serialUSBState.compareTo(WAITING_FOR_RESPONSE) == 0
                    || serialUSBState.compareTo(EXPECTING_BINARY_DATA) == 0) {
            ...
            if (liveRxData.length == 0) {
            ...
            } else if (serialUSBState.compareTo(WAITING_FOR_RESPONSE) == 0
                    || serialUSBState.compareTo(EXPECTING_BINARY_DATA) == 0) {
                ...
                if (binaryBufSize > 0) {
                    ...
                    System.arraycopy(liveRxData, liveRxData.length - binaryBufSize, serialUSBBinaryDataResponse, 0,
                            binaryBufSize);
                } else {
    ...
    /**
     * We seek to generate an exhaustive list of properties and settings for the
     * attached Chameleon device for verbose debugging and error-checking purposes:
     */
    ...
    /****
     * Handle actual communicating with the Chameleon Mini over serial USB. This
     * includes providing an easy-to-use mechanism for translating between the RevE
     * versus RevG variants of the common RevE command set.
     ****/
    ...
    public static ChameleonCommandResult sendRawStringToChameleon(String cmdString, boolean acquireSerialPortLock) {
        ...
        } else if (cmdResult.issuingCmd.length() >= 8
                && cmdResult.issuingCmd.substring(0, 8).equalsIgnoreCase("download")) {
    ...
    public static <CmdArgType> ChameleonCommandResult sendCommandToChameleon(StandardCommandSet cmd,
            CmdArgType cmdArg) {
    ...
    public static enum ChameleonUIDTypeSpec_t {
        TRULY_RANDOM, PREFIXED_RANDOMIZED, INCREMENT_EXISTING, SPECIFY_SUFFIX_BYTES,
    };
    ...
    public static boolean changeChameleonUID(ChameleonUIDTypeSpec_t uidOperation, String suffixBytes) {
        ...
        if (uidOperation == ChameleonUIDTypeSpec_t.INCREMENT_EXISTING) {
            ...
            nextUIDBytes = Utils.generateRandomBytes(chameleonUIDPrefixBytes,
                    chameleonUIDNumBytes - chameleonUIDPrefixBytes.length);
        }
    ...
    /****
     * NFC Tag Types that the Chameleon Boards can emulate (all may not be enabled
     * by default in the standard compile of the firmware...):
     ****/

    /**
     * All possible emulated tag configurations that can be enabled in the RevE
     * Rebooted source.
     * 
    ...
    public static enum ChameleonEmulatedConfigType_t {
        NONE, MF_ULTRALIGHT, MF_ULTRALIGHT_EV1_80B, MF_ULTRALIGHT_EV1_164B, MF_CLASSIC_1K, MF_CLASSIC_1K_7B,
        MF_CLASSIC_4K, MF_CLASSIC_4K_7B, MF_DETECTION, ISO15693_GEN, ISO14443A_SNIFF, ISO14443A_READER, ISO15693_SNIFF,
    ...
    private boolean executeChameleonUpload() {
        ...
        if (XModem.uploadUseInputStream()
                && ChameleonDeviceConfig.THE_CHAMELEON_DEVICE.verifyChameleonUpload(XModem.getUploadInputStream())) {
            return true;
        } else if (!XModem.uploadUseInputStream()
                && ChameleonDeviceConfig.THE_CHAMELEON_DEVICE.verifyChameleonUpload(XModem.getUploadByteStream())) {
            return true;
        } else {
            return false;
        }
